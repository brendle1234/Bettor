        ...
        if winner == 1:
            ...
            if under_over == 0:
                ...
                with open('/content/output.txt', 'a') as file:
                    file.write(Fore.GREEN + home_team + Style.RESET_ALL + Fore.CYAN + f" ({winner_confidence}%)" + Style.RESET_ALL + ' vs ' + Fore.RED + away_team + Style.RESET_ALL + ': ' +
            ...
            if under_over == 0:
                un_confidence = round(ou_predictions_array[count][0][0] * 100, 1)
                with open('/content/output.txt', 'a') as file:
                    file.write(Fore.RED + home_team + Style.RESET_ALL + ' vs ' + Fore.GREEN + away_team + Style.RESET_ALL + Fore.CYAN + f" ({winner_confidence}%)" + Style.RESET_ALL + ': ' +
                      Fore.MAGENTA + 'UNDER ' + Style.RESET_ALL + str(todays_games_uo[count]) + Style.RESET_ALL + Fore.CYAN + f" ({un_confidence}%)" + Style.RESET_ALL)
            else:
                un_confidence = round(ou_predictions_array[count][0][1] * 100, 1)
                with open('/content/output.txt', 'a') as file:
